	...
	"post": function(req, res)
	{
		...
		.exec(function(err, userResult)
		{
			...
			}).exec(function(err, roomResult)
			{
				...
				var post = {};
				post.player = userResult;
				post.character = _.find(req.session.user.characters, { "id": characterID });
				post.room = roomResult;
				...
				}).exec(function(err, postCount)
				{
					...
					Posts.create(post).exec(function(err, savedPost)
					{
						...
						if(!roomResult.posts)
						{
							roomResult.posts = [savedPost];
						}
						else
						{
							roomResult.posts.push(savedPost);
						}
